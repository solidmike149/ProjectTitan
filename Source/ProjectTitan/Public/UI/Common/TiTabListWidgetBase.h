...
#include "TiTabListWidgetBase.generated.h"


USTRUCT(BlueprintType)
struct FTiTabDescriptor
{
	GENERATED_BODY()
...
public:
	...
	void SetTabLabelInfo(const FTiTabDescriptor& TabDescriptor);
};


UCLASS()
class PROJECTTITAN_API UTiTabListWidgetBase : public UCommonTabListWidgetBase
{
	GENERATED_BODY()
...
public:
    ...
    bool GetPreregisteredTabInfo(const FName TabNameId, FTiTabDescriptor& OutTabInfo);

    /** Helper method to get at all the preregistered tab infos */
    const TArray<FTiTabDescriptor>& GetAllPreregisteredTabInfos() { return PreregisteredTabInfoArray; }
    ...
    UFUNCTION(BlueprintCallable, Category = "Tab List")
    bool RegisterDynamicTab(const FTiTabDescriptor& TabDescriptor);
...
private:
    ...
    TArray<FTiTabDescriptor> PreregisteredTabInfoArray;
     ...
     */
    UPROPERTY()
    TMap<FName, FTiTabDescriptor> PendingTabLabelInfoMap;
};
