...
#include "GiltPerformanceStatTypes.h"
#include "Engine/DeveloperSettingsBackedByCVars.h"
#include "Engine/PlatformSettings.h"

#include "GiltPerformanceSettings.generated.h"

// Describes one platform-specific device profile variant that the user can choose from in the UI
USTRUCT()
struct FGiltQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FGiltPerformanceStatGroup
{
	GENERATED_BODY()
...
class UGiltPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UGiltPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UGiltPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FGiltQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	EGiltFramePacingMode FramePacingMode = EGiltFramePacingMode::DesktopStyle;

	// Potential frame rates to display for mobile
	// Note: This is further limited by Gilt.DeviceProfile.Mobile.MaxFrameRate from the
	// platform-specific device profile and what the platform frame pacer reports as supported
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings, meta=(EditCondition="FramePacingMode==EGiltFramePacingMode::MobileStyle", ForceUnits=Hz))
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="Gilt Performance Settings"))
class UGiltPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UGiltPerformanceSettings();
...
public:
	...
	TArray<FGiltPerformanceStatGroup> UserFacingPerformanceStats;
};
